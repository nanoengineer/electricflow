...
let sketch = function (p) {
    ...
    const numOfAmbientCharges = 4;
    const numOfFingerCharges = 5;
    ...
    p.setup = function () {
        ...
        for (let i = 0; i < numOfAmbientCharges + numOfFingerCharges; i++) {
    ...
    p.draw = function () {
        ...
        if (handResultValid(results)) {
            ...
            if (results.handednesses.length == 2) {
                ...
                setHandCharges(lm, p.map(palmOrient, 0, 1, -1, 1));
            }
        }
        else {
            clearHandCharges();
    ...
    class FieldPoint {
        constructor(x, y) {
            ...
            this.potential = 0;
            this.negativeColor = p.color("#faab00");
            this.positiveColor = p.color("#009efa");
        }
        ...
        draw() {
            let vectorDrawLength = p.constrain(this.vector.mag() * 10, 0, 35);
    ...
    function setHandCharges(lm, value) {
        for (let i = numOfAmbientCharges; i < numOfAmbientCharges + numOfFingerCharges; i++) {
            charges[i].position = p.createVector(lm[(i - numOfAmbientCharges + 1) * 4].x * width, lm[(i - numOfAmbientCharges + 1) * 4].y * height);
            charges[i].charge = value;
        }
    }
    function clearHandCharges() {
        for (let i = numOfAmbientCharges; i < numOfAmbientCharges + numOfFingerCharges; i++) {
